...
- **File Dockside (Left Side):** This sits on the left side of the macOS Dock and it is where you can drag and drop items such as files, folders, apps, pdfs, images, videos, and even text snippets from your local machine or from other external sources like web.
![Files Dockside](/files.png)
- **Screenshots and Downloads Dockside (Right Side):** This area sits on the right side of the Dock and it automatically monitors and displays your recent screenshots and downloads, providing easy access to them. **You cannot drag and drop items into this side**—it serves purely as an access point. You can also configure Dockside to monitor the Clipboard for the Screenshots to download them for you for later use.
...
- **Remove When Dragging Out:** Hold the `⌥ — Option` or `⇧ — Shift` key when dragging an item out of Dockside to remove it from Dockside shelf after the operation. This works even when the 'Store only item references' option is enabled. If you want to actually move the file to the target hold the `⌘ - Command` through the drag operation (just like the Finder)
