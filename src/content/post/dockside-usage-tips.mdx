...
- **Remove When Dragging Out:** Hold the `⇧ — Shift` key while dragging an item out of Dockside to remove it from the Dockside shelf after the operation. This works even when the "Store only item references" option is enabled.  
...
- **Ignore Dockside Activation During Drag:** To temporarily prevent Dockside from activating during a drag, press and hold the `Fn` key while dragging.  
...
You can also go the directory where these files are stored and managed by Dockside using the menu option.
